...
problems encountered while using Citrux CLI.
...
your total token usage using the `/stats` command in Citrux CLI.

## Installation and updates

### How do I update Citrux CLI to the latest version?
...
- **Use a `.env` file:** Create a `.env` file in your project's `.citrux`
  directory (`.citrux/.env`) and store your keys there. Citrux CLI will
...
### Where are the Citrux CLI configuration and settings files stored?

The Citrux CLI configuration is stored in two `settings.json` files:

1.  In your home directory: `~/.citrux/settings.json`.
2.  In your project's root directory: `./.citrux/settings.json`.

Refer to [Citrux CLI Configuration](./get-started/configuration.md) for more
details.
...
limits to Gemini Code Assist and Citrux CLI. These are shared across Citrux CLI
...
### What is the privacy policy for using Gemini Code Assist or Citrux CLI if I've subscribed to Google AI Pro or Ultra?
...
across both Gemini 2.5 Pro and Flash. They are shared quota across Citrux CLI
and agent mode in Gemini Code Assist IDE extensions. You can learn more about
quota limits for Citrux CLI, Gemini Code Assist and agent mode in Gemini Code
Assist at
[Quotas and limits](https://developers.google.com/gemini-code-assist/resources/quotas).

### If I upgrade to higher limits for Citrux CLI and Gemini Code Assist by purchasing a Google AI Pro or Ultra subscription, will Gemini start using my data to improve its machine learning models?
...
Search the
[Citrux CLI Q&A discussions on GitHub](https://github.com/google-gemini/gemini-cli/discussions/categories/q-a)
or
