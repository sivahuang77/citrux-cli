...
- **Error:
  ...
  - **Cause:** This error might occur if Citrux CLI detects the
...
- **Error: Command not found (when attempting to run Citrux CLI with
  `gemini`).**
  - **Cause:** Citrux CLI is not correctly installed or it is not in your
    system's `PATH`.
  - **Solution:** The update depends on how you installed Citrux CLI:
    - If you installed `gemini` globally, check that your `npm` global binary
      directory is in your `PATH`. You can update Citrux CLI using the command
    ...
    - If you are running `gemini` from source, ensure you are using the correct
      ...
      update Citrux CLI, pull the latest changes from the repository, and then
...
- **Error: "Operation not permitted", "Permission denied", or similar.**
  - **Cause:** When sandboxing is enabled, Citrux CLI may attempt operations
...
- **Citrux CLI is not running in interactive mode in "CI" environments**
  - **Issue:** The Citrux CLI does not enter interactive mode (no prompt
  ...
  - **Cause:** The `is-in-ci` package checks for the presence of `CI`,
    ...
    non-interactive, which prevents the Citrux CLI from starting in its
    interactive mode.
...
- **DEBUG mode not working from project .env file**
  ...
  - **Solution:** Use a `.citrux/.env` file instead, or configure the
    `advanced.excludedEnvVars` setting in your `settings.json` to exclude fewer
    variables.

## Exit codes

The Citrux CLI uses specific exit codes to indicate the reason for termination.
...
guide_, consider searching the Citrux CLI
