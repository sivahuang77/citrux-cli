...
- **[Memory Import Processor](./memport.md):** Documentation for the modular
  CITRUX.md import feature using @file.md syntax.
...
- **Prompt engineering:** Constructing effective prompts for the Gemini model,
  ...
  instructional context from `CITRUX.md` files.
...
- **API key management:** It handles the `CITRUX_API_KEY` and ensures it's used
...
`CITRUX.md` files that provide context to the model. It searches for these files
...
`refresh` the content of loaded `CITRUX.md` files.
