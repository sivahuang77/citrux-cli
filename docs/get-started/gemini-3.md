# Gemini 3 Pro and Gemini 3 Flash on Citrux CLI

Gemini 3 Pro and Gemini 3 Flash are now available on Citrux CLI! Currently, most
paid customers of Citrux CLI will have access to both Gemini 3 Pro and Gemini 3
...
## How to get started with Gemini 3 on Citrux CLI

Get started by upgrading Citrux CLI to the latest version (0.21.1):
...
1. Use the `/settings` command in Citrux CLI.
2. Toggle **Preview Features** to `true`.
3. Run `/model` and select **Auto (Gemini 3)**.

For more information, see [Citrux CLI model selection](../cli/model.md).

### Usage limits and fallback

Citrux CLI will tell you when you reach your Gemini 3 Pro daily usage limit.
...
Citrux CLI will ask you to decide whether you want to keep trying Gemini 3 Pro
...
When using Citrux CLI, you may want to control how your requests are routed
between models. By default, Citrux CLI uses **Auto** routing.
...
- **Pro routing:** If you want to ensure your task is processed by the most
  capable model, use `/model` and select **Pro**. Citrux CLI will prioritize the
...
[Citrux CLI Model Selection](../cli/model.md).

## How to enable Gemini 3 with Citrux CLI on Gemini Code Assist

If you're using Gemini Code Assist Standard or Gemini Code Assist Enterprise,
enabling Gemini 3 Pro on Citrux CLI requires configuring your release channels.
...
- Navigate to the Google Cloud Project you're using with Citrux CLI for Code
  Assist.
...
Restart Citrux CLI and you should have access to Gemini 3.
