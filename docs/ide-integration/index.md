...
Citrux CLI can integrate with your IDE to provide a more seamless and
...
- **VS Code commands:** You can access Citrux CLI features directly from the VS
  Code Command Palette (`Cmd+Shift+P` or `Ctrl+Shift+P`):
  - `Citrux CLI: Run`: Starts a new Citrux CLI session in the integrated
    terminal.
  - `Citrux CLI: Accept Diff`: Accepts the changes in the active diff editor.
  - `Citrux CLI: Close Diff Editor`: Rejects the changes and closes the active
    diff editor.
  - `Citrux CLI: View Third-Party Notices`: Displays the third-party notices for
    the extension.
...
When you run Citrux CLI inside a supported editor, it will automatically detect
...
manually, you can run the following command inside Citrux CLI:
...
- **For Visual Studio Code:** Install from the
  [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=google.citrux-cli-vscode-ide-companion).
...
> NOTE: The "Citrux CLI Companion" extension may appear towards the bottom of
...
When enabled, Citrux CLI will automatically attempt to connect to the IDE
companion extension.
...
- Open the Command Palette and run **Citrux CLI: Accept Diff**.
...
- Open the Command Palette and run **Citrux CLI: Close Diff Editor**.
...
If you are using Citrux CLI within a sandbox, please be aware of the following:
...
- **On macOS:** The IDE integration requires network access to communicate with
  ...
  network access.
- **In a Docker container:** If you run Citrux CLI inside a Docker (or Podman)
...
- **Message:**
  ...
  - **Cause:** Citrux CLI could not find the necessary environment variables
    (`CITRUX_CLI_IDE_WORKSPACE_PATH` or `CITRUX_CLI_IDE_SERVER_PORT`) to connect
    to the IDE. This usually means the IDE companion extension is not running or
    did not initialize correctly.
  - **Solution:**
    1.  Make sure you have installed the **Citrux CLI Companion** extension in
...
- **Message:**
  `🔴 Disconnected: Directory mismatch. Citrux CLI is running in a different location than the open workspace in [IDE Name]. Please run the CLI from one of the following directories: [List of directories]`
...
- **Message:**
  `IDE integration is not supported in your current environment. To use this feature, run Citrux CLI in one of these supported IDEs: [List of IDEs]`
  - **Cause:** You are running Citrux CLI in a terminal or environment that is
    not a supported IDE.
  - **Solution:** Run Citrux CLI from the integrated terminal of a supported
    IDE, like Antigravity or VS Code.

- **Message:**
  `No installer is available for IDE. Please install the Citrux CLI Companion extension manually from the marketplace.`
  - **Cause:** You ran `/ide install`, but the CLI does not have an automated
    installer for your specific IDE.
  - **Solution:** Open your IDE's extension marketplace, search for "Citrux CLI
    Companion", and
