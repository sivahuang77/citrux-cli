# Provide context with CITRUX.md files

Context files, which use the default name `CITRUX.md`, are a powerful feature
...
1.  **Global context file:**
    - **Location:** `~/.citrux/CITRUX.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Project root and ancestor context files:**
    - **Location:** The CLI searches for a `CITRUX.md` file in your current
...
3.  **Sub-directory context files:**
    - **Location:** The CLI also scans for `CITRUX.md` files in subdirectories
...
### Example `CITRUX.md` file

Here is an example of what you can include in a `CITRUX.md` file at the root of
...
- **`/memory refresh`**: Forces a re-scan and reload of all `CITRUX.md` files
  from all configured locations.
- **`/memory add <text>`**: Appends your text to your global
  `~/.citrux/CITRUX.md` file. This lets you add persistent memories on the fly.

## Modularize context with imports

You can break down large `CITRUX.md` files into smaller, more manageable
...
**Example `CITRUX.md` with imports:**
...
While `CITRUX.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "CITRUX.md"]
  }
}
```
