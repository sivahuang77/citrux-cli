...
describe('memoryCommand', () => {
  ...
  describe('/memory refresh', () => {
    ...
    it('should display success message when memory is refreshed with content (Legacy)', async () => {
      ...
      const refreshResult: LoadServerHierarchicalMemoryResponse = {
        ...
        fileCount: 2,
        filePaths: ['/path/one/CITRUX.md', '/path/two/CITRUX.md'],
      };
  ...
  describe('/memory list', () => {
    ...
    it('should display a message if no CITRUX.md files are found', async () => {
      ...
      expect(mockContext.ui.addItem).toHaveBeenCalledWith(
        {
          type: MessageType.INFO,
          text: 'No CITRUX.md files in use.',
        },
        expect.any(Number),
      );
    });
    ...
    it('should display the file count and paths if they exist', async () => {
      ...
      const filePaths = ['/path/one/CITRUX.md', '/path/two/CITRUX.md'];
      ...
      expect(mockContext.ui.addItem).toHaveBeenCalledWith(
        {
          type: MessageType.INFO,
          text: `There are 2 CITRUX.md file(s) in use:\n\n${filePaths.join('\n')}`,
        },
        expect.any(Number),
      );
    });
