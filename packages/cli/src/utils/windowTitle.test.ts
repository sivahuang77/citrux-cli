...
describe('computeWindowTitle', () => {
  ...
  it('should use default Citrux title when CLI_TITLE is not set', () => {
    const result = computeWindowTitle('my-project');
    expect(result).toBe('Citrux - my-project');
  });
  ...
  it('should handle folder names with special characters', () => {
    ...
    expect(result).toBe('Citrux - project-name_v1.0');
  });
});
